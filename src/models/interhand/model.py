...
from src.models.init_regressor.model import InitRegressor
...
class InterHand(nn.Module):
    def __init__(self, backbone, args):
        ...
        self.pose_regressor = Regressor(args.focal_length, args.img_res)

        self.init_regressor = InitRegressor(backbone, self.pose_regressor)

        self.arctic_model = ArcticSF(backbone, self.pose_regressor)

        # self.field_model = FieldSF(backbone)
    ...
    def forward(self, inputs, meta_info, models):
        #feature extraction
        images = inputs["img"]#64,3,224,224
        features = self.backbone(images)

        # # uncomment to use ViT backbone
        ...
        #init regressor
        init_result = self.init_regressor(features, meta_info)

        # updated_field = self.field_model(features, meta_info, init_result, models['mesh_sampler'])

        # #hand/object pose prediction
        output = self.arctic_model(features, meta_info, init_result, None)
