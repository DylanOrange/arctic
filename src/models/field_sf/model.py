...
class FieldSF(nn.Module):
    ...
    def forward(self, inputs, meta_info):
        ...
        points_r = meta_info["v0.r"].permute(0, 2, 1)[:, :, 21:]#64,3,195, exclude keyjoints
        points_l = meta_info["v0.l"].permute(0, 2, 1)[:, :, 21:]#64,3,195
        points_o = meta_info["v0.o"].permute(0, 2, 1)#64,3,600
        points_all = torch.cat((points_r, points_l, points_o), dim=2)#64,3,990
        ...
        #refinement
        dist_o = torch.stack([dist_or, dist_ol], dim=1)

        dist_feat_r = self.point_backbone_h(dist_ro[:,None,:])[0]#64,256,195
        dist_feat_l = self.point_backbone_h(dist_lo[:,None,:])[0]
        ...
        #upsampling
        dist_ro = self.upsampling_r((dist_ro+dist_ro_residual)[:, :, None])[:, :, 0]#64,778
        dist_lo = self.upsampling_l((dist_lo+dist_lo_residual)[:, :, None])[:, :, 0]#64,778
        dist_or = self.upsampling_o((dist_or+dist_or_residual)[:, :, None])[:, :, 0]#64,4000
        dist_ol = self.upsampling_o((dist_ol+dist_ol_residual)[:, :, None])[:, :, 0]#64,4000
