...
def get_selected_seqs(setup, split):
    ...
    with open(
        op.join("/storage/user/lud/dataset/arctic/data/arctic_data/data/splits_json/", f"protocol_{setup}.json"), "r"
    ) as f:
        splits = json.load(f)
...
def glob_fnames(num_frames, seq, chosen_views):
    ...
    folder_p = op.join(f"/storage/user/lud/dataset/arctic/data/arctic_data/data/images/{sid}/{seq_name}/")
