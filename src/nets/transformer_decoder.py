...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        k, v = self.to_kv(context).chunk(2, dim=-1)#2,192,512
        q = self.to_q(x)#2,1,512
        q, k, v = map(lambda t: rearrange(t, "b n (h d) -> b h n d", h=self.heads), [q, k, v])

        #q:2,8,1,64, k:2,8,192,64, v:2,8,192,64
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale

        attn = self.attend(dots)#2,8,1,192->2,8,1,192
        attn = self.dropout(attn)

        out = torch.matmul(attn, v)#2,8,1,64
        out = rearrange(out, "b h n d -> b n (h d)")#2,1,512
        return self.to_out(out)#2,1,1024
...
class TransformerCrossAttn(nn.Module):
    ...
    def forward(self, x: torch.Tensor, *args, context=None, context_list=None):
        ...
        for i, (self_attn, cross_attn, ff) in enumerate(self.layers):
            x = self_attn(x, *args) + x#2,1,1024
            x = cross_attn(x, *args, context=context_list[i]) + x#2,1,1024
            x = ff(x, *args) + x
        return x
...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, num_tokens, dim))#1,1,1024
        if emb_dropout_type == "drop":
            self.dropout = DropTokenDropout(emb_dropout)
        elif emb_dropout_type == "zero":
            self.dropout = ZeroTokenDropout(emb_dropout)
    ...
    def forward(self, inp: torch.Tensor, *args, context=None, context_list=None):
        x = self.to_token_embedding(inp)#2,1,1024
        b, n, _ = x.shape
